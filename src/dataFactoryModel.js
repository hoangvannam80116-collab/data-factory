...
const buildStoreMetricRule = ({ id, fieldName, value, valueType = '数字文本' }) => ({
  ...
  fieldName,
  value,
  prompt: `进入千牛商家工作台首页后先刷新页面，等待「店铺数据」模块和「数据更新时间」更新完成，再读取「${fieldName}」卡片里的当前主数值。只返回${valueType}，不要读取昨日值或刷新前旧值。`,
...
export const buildTabbitBatchPrompt = ({ shop, rules }) => {
  ...
  return [
    ...
    '3. 校准通过后必须先刷新当前页面一次，等待「店铺数据」区域重新渲染，并读取页面显示的「数据更新时间」。不要使用刷新前 DOM、旧截图、聊天记录或历史结果。',
    '4. 刷新完成后一次性读取下面所有字段，只读取当前主数值，不读取昨日值；不要猜测，不确定就标 error。',
    fieldLines,
    '',
    '数据 freshness 要求:',
    '- evidence 必须说明已经刷新页面，并写明读取到的「数据更新时间」。',
